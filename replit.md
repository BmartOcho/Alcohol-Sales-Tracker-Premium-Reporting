...
- **Automated Data Refresh System (Jan 2025)**: Implemented incremental data updates from Texas Open Data API. Admin UI with DataRefreshDialog component in user menu for paid users to trigger manual updates. `/api/admin/refresh-data` endpoint with `/api/admin/data-status` monitoring. Fixed critical same-day update bug by starting from latest date (not +1 day) to capture same-day API additions. Application-level duplicate filtering prevents re-importing existing records. **Known Limitation**: Minor race condition exists if multiple admins trigger refresh simultaneously (0.12% legacy duplicates from development exist in database). Future enhancement: Add unique database constraint during maintenance window.
