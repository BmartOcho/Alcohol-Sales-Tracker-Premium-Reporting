...
- **Real-Time Data Refresh System (Nov 2025)**: Completely redesigned data refresh architecture for 100% accuracy and real-time updates. **Startup Refresh**: Server now runs `importIncrementalData()` on every startup to catch any missed updates immediately. **Hourly Auto-Refresh**: Cron scheduler checks for new monthly reports every hour (changed from daily at 3 AM) to ensure data is never more than 1 hour old. **Automatic Cache Invalidation**: Location cache is cleared after each successful import, ensuring users see fresh data instantly without manual intervention. **Comprehensive Logging**: ISO timestamps track every refresh operation with detailed import counts and latest data dates. System now maintains perfect parity with Texas Open Data Portal, pulling historical data and new monthly reports automatically with zero manual intervention required.
- **Search UX Enhancement (Jan 2025)**: Improved permit search experience in Reports > Permit Report tab. Increased backend search result limit from 20 to 50 locations (still ordered by total sales DESC). Replaced basic loading skeletons with engaging SearchLoadingAnimation component featuring 5 pulsing bars with staggered wave effect and contextual loading messages. Uses framer-motion for smooth animations. Applied to both "Searching locations..." and "Loading location details..." states for consistent, professional user feedback during database queries.
- **Freemium Data Accuracy Fix (Jan 2025)**: Fixed critical bug where free users received random locations instead of the actual top 10 highest-grossing establishments. Added server-side sorting by `totalSales` descending in `/api/locations` endpoint after all filtering but before pagination. This ensures free tier shows genuine preview data - verified via database queries that Travis County top 10 matches exactly between free and paid users. Both tiers now use identical data processing pipeline; only difference is quantity restrictions (free: 10 results, paid: unlimited).
