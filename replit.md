...
- **Freemium Paywall (Jan 2025)**: Implemented freemium model with 3 free manual searches per day for unauthenticated users. Home page is now publicly accessible without login. Features include: localStorage-based tracking with daily midnight reset, search counting only on Enter key press (not keystroke), green numbered badges (#1-#10) for top 10 locations, blur effect on sales data for locations ranked 11+, non-dismissible paywall modal after 3rd search with Sign In/Sign Up options, and clear UI hint "Press Enter to search" to indicate how searches are counted.
